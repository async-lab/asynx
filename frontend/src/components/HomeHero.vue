<template>
  <section class="home-hero" :class="compact ? 'compact' : ''">
    ...
    <nav class="hero-links" v-if="resolvedLinks.length">
      <a v-for="link in resolvedLinks" :key="link.href" class="hero-link" :href="link.href" target="_blank" rel="noopener">
        {{ link.label }}
      </a>
    </nav>
...
const resolvedLinks = computed(() => Array.isArray(props.links) ? props.links : [])
...
.home-hero {
  ...
  background:
    ...
    radial-gradient(900px 600px at 8% 12%, rgba(64,158,255,0.12), transparent 60%),
    radial-gradient(700px 420px at 92% 80%, rgba(103,194,58,0.12), transparent 60%),
...
.home-hero::before {
  ...
  inset: 0;
  background-image:
    linear-gradient(to right, rgba(64,158,255,0.08) 1px, transparent 1px),
    linear-gradient(to bottom, rgba(64,158,255,0.08) 1px, transparent 1px);
  background-size: 22px 22px;
  mask-image: radial-gradient(600px 400px at 20% 20%, rgba(0,0,0,0.6), transparent 70%);
  pointer-events: none;
  z-index: 0;
}
...
.hero-logo { width: 120px; height: 120px; border-radius: 18px; }
.hero-texts { display: flex; flex-direction: column; align-items: center; text-align: center; position: relative; z-index: 1; }
.hero-title { margin: 0; color: #1f2d3d; font-size: 36px; line-height: 1.15; letter-spacing: 0.2px; }
.hero-subtitle { margin: 8px 0 0 0; color: #5e6d82; font-size: 16px; }

.hero-links { position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); display: flex; gap: 12px; flex-wrap: wrap; justify-content: center; z-index: 1; }
.hero-link {
  color: #2e6ee9; text-decoration: none; padding: 8px 14px; border-radius: 999px;
  border: 1px solid rgba(46,110,233,0.16); background: rgba(46,110,233,0.06);
  transition: all .2s ease; font-weight: 600;
}
.hero-link:hover { color: #fff; background: linear-gradient(135deg, #3a81ff, #66b3ff); border-color: transparent; transform: translateY(-1px); box-shadow: 0 6px 14px rgba(58,129,255,0.25); }
...
@media (max-width: 768px) {
  .home-hero { padding: 16px; border-radius: 12px; gap: 10px; }
